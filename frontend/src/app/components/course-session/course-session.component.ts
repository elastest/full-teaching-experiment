...
import {Connection, OpenVidu, Publisher, Session, Stream} from 'openvidu-angular';
...
export class CourseSessionComponent implements OnInit {
  ...
  joinSession() {
    ...
    this.videoSessionService.getSessionIdAndToken(this.OVSessionId).subscribe(response => {
      ...
      this.OVSession.on('streamCreated', (event: any) => {
      ...
      this.OVSession.on('streamDestroyed', (event: any) => {
      ...
      this.OVSession.on('connectionCreated', (event: any) => {
      ...
      this.OVSession.on('connectionDestroyed', (event: any) => {
      ...
      // Signals
      this.OVSession.on('signal:chat', (msg: any) => {
      ...
      if (!this.authenticationService.isTeacher()) {
        this.OVSession.on('signal:grantIntervention', (msg: any) => {
          if (msg.data === 'true') {
            // Publish
            this.publish();
            this.studentAccessGranted = true;
      ...
      if (this.authenticationService.isTeacher()) {
        this.OVSession.on('signal:askIntervention', (msg: any) => {
  ...
  publish() {
    this.OVPublisher.on('streamCreated', (event: any) => {
    ...
    this.OVPublisher.on('videoElementCreated', (event: any) => {
