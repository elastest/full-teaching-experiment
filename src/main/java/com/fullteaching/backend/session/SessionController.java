...
public class SessionController {
	...
	public ResponseEntity<Object> modifySession(@RequestBody Session session) {
		...
		Session s = sessionRepository.findById(session.getId()).get();

		log.info("Updating session. Previous value: {}", s.toString());

		ResponseEntity<Object> teacherAuthorized = authorizationService.checkAuthorization(s,
				s.getCourse().getTeacher());
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			return teacherAuthorized;
		} else {
	...
	public ResponseEntity<Object> deleteSession(@PathVariable(value = "id") String id) {
		...
		Session session = sessionRepository.findById(id_i).get();
		...
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			Course course = courseRepository.findById(session.getCourse().getId()).get();
			if (course != null) {
				...
				// This code is presumed to be unreachable, because of the Cascade.ALL
				// relationship from Course to Session
