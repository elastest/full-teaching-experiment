...
public class FileGroupController {
	...
	public ResponseEntity<Object> newFileGroup(@RequestBody FileGroup fileGroup,
			@PathVariable(value = "id") String courseDetailsId) {
		...
		ResponseEntity<Object> teacherAuthorized = authorizationService.checkAuthorization(cd,
				cd.getCourse().getTeacher());
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			// fileGroup is a child of an existing FileGroup
				...
				FileGroup fParent = fileGroupRepository.findById(fileGroup.getFileGroupParent().getId()).get();
				if (fParent != null) {
					fParent.getFileGroups().add(fileGroup);
					/*
					 * Saving the modified parent FileGroup: Cascade relationship between FileGroup
					 * and its FileGroup children will add the new fileGroup to FileGroupRepository
					 */
					fileGroupRepository.save(fParent);
					CourseDetails cd2 = courseDetailsRepository.findById(id_i).get();
	...
	public ResponseEntity<Object> modifyFileGroup(@RequestBody FileGroup fileGroup,
		...
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			FileGroup fg = fileGroupRepository.findById(fileGroup.getId()).get();
	...
	public ResponseEntity<Object> deleteFileGroup(@PathVariable(value = "fileGroupId") String fileGroupId,
			@PathVariable(value = "courseId") String courseId) {
		...
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			FileGroup fg = fileGroupRepository.findById(id_fileGroup).get();
	...
	public ResponseEntity<Object> editFileOrder(@PathVariable(value = "courseId") String courseId,
			@PathVariable(value = "fileId") String fileId, @PathVariable(value = "sourceID") String sourceId,
			@PathVariable(value = "targetId") String targetId, @PathVariable(value = "position") String position) {
		...
		try {
			...
			log.error(
					"Course ID '{}' or File ID '{}' or source ID '{}' or target ID '{}' are not of type Long; or position is not of type Integer",
		...
		if (teacherAuthorized != null) { // If the user is not the teacher of the course
			...
			FileGroup sourceFg = fileGroupRepository.findById(id_source).get();
			FileGroup targetFg = fileGroupRepository.findById(id_target).get();
			File fileMoved = fileRepository.findById(id_file).get();

			log.info("Moving file {} from filegroup {} to filegroup {} into position {}", fileMoved, sourceFg, targetFg,
					pos);
	...
	public ResponseEntity<Object> modifyFile(@RequestBody File file,
	...
	public ResponseEntity<Object> deleteFile(@PathVariable(value = "fileId") String fileId,
		...
		try {
			...
			log.error("Course ID '{}' or FileGroup ID '{}' or File ID '{}' are not of type Long", courseId, fileGroupId,
					fileId);
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
		}
	...
	// Method to get the root FileGroup of a FileGroup tree structure, given a
	// FileGroup
