...
public class CommentController {
	...
	public ResponseEntity<Object> newComment(
		...
		CourseDetails cd = courseDetailsRepository.findById(id_courseDetails).get();
		...
		if (userAuthorized != null) { // If the user is not an attender of the course
			...
			if (comment.getCommentParent() == null) {
				...
				Entry entry = entryRepository.findById(id_entry).get();
				if(entry != null) {
			...
			//The comment is a replay to another existing comment
				...
				Comment cParent = commentRepository.findById(comment.getCommentParent().getId()).get();
				if(cParent != null){
